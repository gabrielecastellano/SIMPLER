...
class Agent:
    def __init__(self, name, model=None):
        self.name = name
        self.id = self.name + '_' + ''.join(random.choice(string.ascii_lowercase) for _ in range(5))
        self.model = model
        self.points = 0
    ...
    def print_top_actions(action_probs):
        ...
        logger.debug(f"Top 5 actions: "
                     f"{[str(i) + ': ' + str(round(a, 2))[:5] for i, a in zip(top5_action_idx, top5_actions)]}")
